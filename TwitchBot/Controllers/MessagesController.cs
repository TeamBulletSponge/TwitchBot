      ...
      if (message.Type == "Message")
      {
        ...
        if (tokens[0] == "/status")
        {
          ...
          if (result["stream"].HasValues)
          {
            ...
            Message reply = message.CreateReplyMessage(String.Format("[{0}]({1}{0}) is online since {2}{0}.{3}", channel, "http://twitch.tv/", result["stream"]["created_at"].ToString(), Environment.NewLine));
